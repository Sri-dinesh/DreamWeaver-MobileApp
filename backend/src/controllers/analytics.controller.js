...
const getDreamConsistency = async (req, res) => {
    try {
        ...
        if (dreams.length < 2) {
            return res.status(200).json({ message: 'Not enough dream entries to calculate consistency.' });
        }

        const timeDiffs = [];
        for (let i = 1; i < dreams.length; i++) {
            const diff = (dreams[i].timestamp.getTime() - dreams[i-1].timestamp.getTime()) / (1000 * 60 * 60 * 24);
            timeDiffs.push(diff);
        }

        const avgDiff = timeDiffs.reduce((a, b) => a + b, 0) / timeDiffs.length;
        const consistency = 1 / (1 + avgDiff); // A simple consistency score
...
const getEmotionalSleepMap = async (req, res) => {
    try {
        ...
        if (dreams.length === 0) {
            return res.status(200).json({ message: 'No dream entries to create an emotional sleep map.' });
        }
...
const getDreamEmotionsDistribution = async (req, res) => {
    try {
        ...
        if (dreams.length === 0) {
            return res.status(200).json({ message: 'No dream entries to calculate emotion distribution.' });
        }
...
const getSleepDuration = async (req, res) => {
    try {
        ...
        if (sleepPlans.length === 0) {
            return res.status(200).json({ message: 'No sleep duration data available for the last 30 days.' });
        }

        const sleepDurations = sleepPlans.map(plan => ({
            date: plan.plan_date,
            duration: (plan.wake_time.getTime() - plan.sleep_time.getTime()) / (1000 * 60 * 60), // in hours
        }));
...
const getLucidDreamsPerDay = async (req, res) => {
    try {
        ...
        if (lucidDreams.length === 0) {
            return res.status(200).json({ message: 'No lucid dream data available.' });
        }
...
const getSleepDreamCorrelations = async (req, res) => {
    try {
        ...
        if (sleepPlans.length < 2 || dreams.length < 2) {
            return res.status(200).json({ message: 'Not enough combined sleep and dream data to calculate correlations.' });
        }

        const sleepData = sleepPlans.map(plan => ({
            date: plan.plan_date.toISOString().split('T')[0],
            duration: (plan.wake_time.getTime() - plan.sleep_time.getTime()) / (1000 * 60 * 60),
        }));
        ...
        const correlation = pearsonCorrelation(combinedData.map(d => d.duration), combinedData.map(d => d.lucidCount));
